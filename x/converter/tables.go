...
func GetEnumsFromJSON(mudConfigJSON []byte) []Enum {
	...
	err := jsonparser.ObjectEach(mudConfigJSON, func(key []byte, value []byte, dataType jsonparser.ValueType, offset int) error {
		...
		if dataType == jsonparser.Array {
			_, err := jsonparser.ArrayEach(mudConfigJSON, func(value []byte, dataType jsonparser.ValueType, offset int, err error) {
				e.Values = append(e.Values, string(value))
			}, configName, enumsName, string(key))
			if err != nil {
				return err
			}
		}
